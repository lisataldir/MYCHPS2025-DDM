...
disp(concat_ub);
disp(iter);
% Supposons que vous avez déjà défini Sd_assembled, bd_assembled, et G

% Initialisation du préconditionneur de Dirichlet
% Supposons que les conditions de Dirichlet sont appliquées au premier nœud
fixed_dofs = 1; % Par exemple, le premier degré de liberté est fixé

% Construire le préconditionneur de Dirichlet
M_D = Sd_assembled; % Utiliser la matrice de rigidité assemblée
M_D(fixed_dofs, :) = []; % Enlever la ligne correspondante
M_D(:, fixed_dofs) = []; % Enlever la colonne correspondante

% Calculer l'inverse du préconditionneur
M_D_inv = inv(M_D);

% Gradient conjugué avec préconditionneur de Dirichlet
% Initialisation
lambda = - G * ((G' * G) \ e);
r = P' * (-bd_assembled - Sd_assembled * lambda);
z = P * (M_D_inv * r); % Utiliser l'inverse du préconditionneur
d = z;
...
% Initialisation du préconditionneur de Dirichlet
% Supposons que les conditions de Dirichlet sont appliquées au premier nœud
fixed_dofs = 1; %  le premier degré de liberté est fixé
...
while (norm(r) > 1e-3)
    ...
    lambda = lambda + alpha * d;
    r = r - alpha * p;
    z = P * (M_D_inv * r); %l'inverse du préconditionneur
    beta = -z' * p / (d' * p);
    d = z + beta * d;
